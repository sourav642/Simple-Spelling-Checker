...
template<class Items>
Dictionary<Items>::Dictionary(int hashSze)
{
	hashSize = hashSze;
...
template<class Items>
bool Dictionary<Items>::isEmpty(int key)
{
...
template<class Items>
int Dictionary<Items>::keygen(Items newItem)
{
...
template<class Items>
void Dictionary<Items>::insert(Items newItem)
{
	if(isFull())
	{
        cout<<"\nnError:TABLE FULL\n";
...
template<class Items>
void Dictionary<Items>::append(int key, Items newItem)
{
	if(isFull())
	{
        cout<<"\nERROR:TABLE FULL\n";
...
template<class Items>
bool Dictionary<Items>::remove(Items deleteItem, int key)
{
	bool isFound = false;
	list_node* tempPtr;
	if(key == -1)
...
template<class Items>
int Dictionary<Items>::table_size()
{
    return hashSize;
}
template<class Items>
int Dictionary<Items>::total_elements()
{
    return totElems;
}

template<class Items>
int Dictionary<Items>::bsize(int key)
{
	return(!isEmpty(key)) ? bucket_size[key]:0;
}

template<class Items>
int Dictionary<Items>::search_table(Items searchItem)
{
...
template <class Items>
class Dictionary<Items>::Iterator :
